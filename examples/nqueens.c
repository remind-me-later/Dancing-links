...
// row, column or diagonal. For example a solution for n = 4:
//
// ```
//   +---+---+---+---+
// 4 |   | ♕ |   |   |
...
// down left, also we will consider only diagonals that cover more than a 
// single square, an example can be seen in the following illustration 
// for n = 4, with
//
// | Constraint       | Symbol |
// | ---------------- | ------ |
...
// The parentheses are important on the following macro definition, let's see
// why with an example.Without the parentheses with n = 4 for the corners 
// travesed by the central reverse diagonal a1 (0,0) and d4 (N - 1, N - 1) we
// have
//
// ```
//                      B3
